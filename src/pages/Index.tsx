...
const Index = () => {
    ...
    const loadDiseases = async () => {
      try {
        const {
          data,
          error
        } = await supabase.functions.invoke('diseases-meta');
  ...
  const getSelectedDiseaseNames = () => {
    ...
    return selectedDiseases.map(id => diseaseOptions.find(d => d.id === id)?.name || id).join(", ");
  };
    ...
    return <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 flex flex-col">
        <header className="w-full py-16 px-4 text-center relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <h1 className="font-bold text-foreground mb-4 tracking-tight text-3xl">
              내 건강 정보 입력
            </h1>
            ...
            <CardContent className="p-10 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input id="age" type="number" min="0" placeholder="예: 35" value={age} onChange={e => setAge(e.target.value)} className="h-12 text-base" />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select id="gender" value={gender} onChange={e => setGender(e.target.value)} className="w-full h-12 px-3 rounded-md border-2 border-input bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-ring text-base">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="height" type="number" min="0" placeholder="예: 170" value={height} onChange={e => setHeight(e.target.value)} className="h-12 text-base" />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="weight" type="number" step="0.1" min="0" placeholder="예: 65.5" value={weight} onChange={e => setWeight(e.target.value)} className="h-12 text-base" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <select id="diseases" multiple value={selectedDiseases} onChange={e => {
                ...
                setSelectedDiseases(selected);
                  ...
                  {diseaseOptions.map(disease => <option key={disease.id} value={disease.id} className="py-2">
                      {disease.name}
                    </option>)}
                </select>
              ...
              <div className="flex flex-col gap-4 mt-8">
                <Button onClick={handleProceedToAnalyze} disabled={!age || !gender || !height || !weight} className="w-full h-14 text-lg font-bold shadow-lg hover:shadow-xl transition-all hover:scale-[1.02]">
                  🍽️ 식단 체크하기
                </Button>
                <Button onClick={() => navigate("/my-diet")} variant="outline" className="w-full h-14 text-lg font-bold shadow-lg hover:shadow-xl transition-all hover:scale-[1.02]">
                  📋 내 식단 보기
                </Button>
              </div>
            </CardContent>
      ...
      <main className="flex-1 flex items-center justify-center px-4 pb-12">
        <Card className="w-full max-w-3xl shadow-2xl border-2 bg-card/95 backdrop-blur">
          <CardContent className="p-10 space-y-8">
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1">
                    {!hidePersonal ? <div className="space-y-1">
                      ...
                      </div> : <p className="text-base text-muted-foreground italic">
                        개인 정보가 숨김 처리되었습니다.
                      </p>}
                  </div>
                  <div className="flex flex-col gap-2">
                    <Button onClick={() => setHidePersonal(!hidePersonal)} variant="outline" size="sm" className="whitespace-nowrap">
                      {hidePersonal ? "👁️ 개인 정보 보이기" : "🙈 개인 정보 숨기기"}
                    </Button>
                    <Button onClick={handleBackToIntro} variant="outline" size="sm" className="whitespace-nowrap">
                      ✏️ 정보 다시 입력
                    </Button>
                  </div>
                </div>
            ...
            {/* Image Upload Area */}
            <div onClick={handleUploadAreaClick} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver} onDrop={handleDrop} className={`
                relative w-full rounded-xl border-2 border-dashed
                flex items-center justify-center cursor-pointer
                transition-all duration-300 overflow-hidden
                ${previewUrl ? 'min-h-[400px] max-h-[600px]' : 'min-h-[400px]'}
                ${isDragging ? "border-primary bg-primary/20 scale-[1.02] shadow-2xl" : previewUrl ? "border-primary bg-primary/5 shadow-inner" : "border-upload-border bg-upload-bg hover:border-primary hover:bg-primary/10 hover:shadow-lg"}
              `}>
              <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileInputChange} className="hidden" />

              {previewUrl ? <div className="w-full h-full flex items-center justify-center p-4">
                  <img src={previewUrl} alt="업로드된 이미지 미리보기" className="max-w-full max-h-[560px] object-contain rounded-lg shadow-lg" />
            ...
            <div className="flex gap-4">
              <Button onClick={handleAnalyze} disabled={isLoading} className="flex-1 h-14 text-lg font-bold shadow-lg hover:shadow-xl transition-all hover:scale-[1.02]">
                {isLoading ? <span className="flex items-center gap-2">
                    <span className="animate-spin">⏳</span>
                    분석 중...
                  </span> : "🔍 분석하기"}
              </Button>
              {result && <Button onClick={handleSave} variant="secondary" className="h-14 px-10 text-lg font-semibold shadow-lg hover:shadow-xl transition-all hover:scale-[1.02]">
                  <Save className="mr-2 h-5 w-5" />
                  저장
                </Button>}
              <Button onClick={handleReset} variant="outline" className="h-14 px-10 text-lg font-semibold hover:bg-secondary/50 transition-all hover:scale-[1.02]">
                🔄 초기화
              </Button>
            </div>
            ...
            <Card className="bg-gradient-to-br from-secondary/60 to-secondary/30 border-2 border-secondary shadow-lg">
              <CardContent className="p-8">
                ...
                <div className="min-h-[100px] flex flex-col gap-3 justify-center rounded-lg bg-card/50 p-6">
                  {error ? <p className="text-destructive text-lg font-semibold flex items-center gap-2">
                      <span>❌</span> {error}
                    </p> : result ? <>
                      <p className="text-2xl font-bold text-primary">
                        음식: {result}
                      </p>
                      {calories && <p className="text-lg font-medium text-foreground">
                          대략적인 칼로리: {calories}
                        </p>}
            ...
            {/* Risk Box */}
            {riskLevel && <Card className={`border-2 shadow-lg ${riskLevel === "안전" ? "bg-green-50 dark:bg-green-950/20 border-green-500" : riskLevel === "위험" ? "bg-red-50 dark:bg-red-950/20 border-red-500" : "bg-yellow-50 dark:bg-yellow-950/20 border-yellow-500"}`}>
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className={`text-xl font-bold mb-2 ${riskLevel === "안전" ? "text-green-700 dark:text-green-300" : riskLevel === "위험" ? "text-red-700 dark:text-red-300" : "text-yellow-700 dark:text-yellow-300"}`}>
                        {riskLevel === "안전" ? "안전" : riskLevel === "위험" ? "위험" : "주의"}
                      </h3>
                      {riskComment && <p className="text-base text-foreground/90">
                          {riskComment}
                        </p>}
                    </div>
                  </div>
